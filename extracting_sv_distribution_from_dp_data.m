...
filename = uigetfile(('*.mat'),'MultiSelect', 'on', 'Select matlab file', directory);
...
for i = 1:length(ordered_fields)
    ...
    [a,b] = size (sample_in.(ordered_fields{i}).vesicle_distribution_dp.all_SV);
    c=1;
    ...
    for k = 7:b
        ...
        for j = 1:a
            ...
            temp_table_1 (j, c) = sample_in.(ordered_fields{i}).vesicle_distribution_dp.all_SV (j,k);
            
            
            %treating distribution from each image as independent values
            temp_table_2 (j, d) = sample_in.(ordered_fields{i}).vesicle_distribution_dp.all_SV (j,k);
    ...
    for j = 1:a
        ...
        norm_abundance (j,i+1) = sample_in.(ordered_fields{i}).vesicle_distribution_dp.all_SV(j,6);
    end
