            ...
            if isfield (filename.raw_data(i).distance_data, 'vesicle')
                for sv = 1:length(filename.raw_data(i).distance_data.vesicle)
                    ...
                    if strcmp(type, 'particle') 
                        ...
                        if filename.raw_data(i).distance_data.vesicle(sv).dist2az <= 100 && filename.raw_data(i).distance_data.vesicle(sv).closest_az == h
...
%    if isfield(filename.raw_data(i).analysis_data, 'pits')&&isfield(filename.raw_data(i).analysis_data, 'vesicle')
    if isfield(filename.raw_data(i).analysis_data, 'vesicle') 
    ...
    for k = 1:length(filename.raw_data(i).analysis_data.vesicle)
        ...
        if strcmp (type, 'particle')
            ...
            z_half = (length(filename.raw_data) + length(missing_sect))*sect_thick/2;
            %distance to edge in z direction
            z_edge_dist = abs(z_half - z_cent_dist);
            z_dist_norm = z_edge_dist/(z_half);
...
%             end
            ...
            %separate rows for separate AZs
                ...
                vesicle_data(a,2+(c-1)*5) = sqrt(z_cent_dist^2 + pit_az_cent^2); %distance from center of AZ
                
                if reconstruct_tf == 1
                    vesicle_data(a,3+(c-1)*5) = (1-xy_dist_norm)^2; % distance to center normalized by fractional area for 3D dataset
                else
                    vesicle_data(a,3+(c-1)*5) = 1-xy_dist_norm; %distance to center, normalized by radius of active zone
                end
