function pits_size_data = average_fpits_diameter (data, analysis_type)
...
if strcmp(analysis_type, 'total')
    ...
    for i = 1:length(data)
        ...
        if isfield(data(i).analysis_data, 'fpits')
            
            a = a+1;
            
            b = b+length(data(i).analysis_data.fpits);
    ...
    for i = 1:length(data)
        ...
        if isfield(data(i).analysis_data, 'fpits')
            ...
            for j = 1:length(data(i).distance_data.fpits)
                ...
                if data(i).distance_data.fpits(j).in_az == 0
...
if strcmp (analysis_type, 'total')
    ...
    for i = 1:length(data)
        ...
        if isfield(data(i).analysis_data, 'fpits')
            ...
            for j = 1:length(data(i).analysis_data.fpits)
                
                pits_size_data.average_total(1,m) = data(i).analysis_data.fpits(j).neck_width;
                pits_size_data.average_total(2,m) = data(i).analysis_data.fpits(j).height;
                pits_size_data.average_total(3,m) = data(i).analysis_data.fpits(j).diameter;
                pits_size_data.average_total(4,m) = data(i).analysis_data.fpits(j).length;
                pits_size_data.average_total(5,m) = data(i).analysis_data.fpits(j).surface_area;
