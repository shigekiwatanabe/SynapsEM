...
for i = 1:length(ordered_fields) 
    ...
    if length(strfind(ordered_fields{i}, 'output')) == 0
        ...
        sample_out.(name_list{i}) = pits_reconstruct(sample_in.(ordered_fields{i}), reconstruct_tf, pix_size, sect_thick, height_cut);
    end
    
    if isempty (sample_out.(name_list{i}).PitData) ==0
        
        [row, col] = size(sample_out.(name_list{i}).PitData);
        sv_in_row = col/5;
        ...
        for m = 1:sv_in_row
            ...
            for j= 1:row
                ...
                if sample_out.(name_list{i}).PitData(j,(5*(m-1)+1)) >0 ...
                        && sample_out.(name_list{i}).PitData(j,(5*(m-1)+1)) ~= Inf
                ...
                for m=1:sv_in_row
                    ...
                    for j = 1:row
                        ...
                        if sample_out.(name_list{i}).PitData(j,(5*(m-1)+1)) >0
                            ...
                            if sample_out.(name_list{i}).PitData(a(k),b(k)+3)== sample_out.(name_list{i}).PitData(j,(5*m))
                                
                                x_dist = abs(sample_out.(name_list{i}).PitData(a(k),b(k))...
                                    -     sample_out.(name_list{i}).PitData(j,(5*(m-1)+2)));
                                
                            elseif sample_out.(name_list{i}).PitData(a(k),b(k)+3)~= sample_out.(name_list{i}).PitData(j,(5*m))
...
clear directory filename all_fields i name_list height_cut pix_size sect_thick reconstruct_tf index_order ordered_fields col j k m row sv_in_row